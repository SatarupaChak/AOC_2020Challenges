...
def getRow(value, startVal,endVal):
    if value =='F':
        ...
        if startVal == 0 and endVal == 127:
            print('secondHalf')
            newStartVal = ((endVal+1)//2)
            return newStartVal,endVal
        else:
           newStartVal = (endVal-(((endVal-startVal)+1)//2))+1
           return newStartVal,endVal

def getMe(x):
    newRowString = x[(len(x) - 3):len(x)]
    newColStart = None
    endCol = None
    for x in newRowString:
            if newColStart is None and endCol is None:
                val1, val2 = getColumn(x, 0, 7)
                newColStart = val1
                endCol = val2
            elif newColStart is not None and endCol is not None:
                val1, val2 = getColumn(x, newColStart, endCol)
                newColStart = val1
                endCol = val2
    if newColStart is not None and newColStart == endCol:
       return newColStart




def getColumn(charec,start,end):
    if charec == 'R':
        if start == '0' and end =='7':
            newStart = (end+1) // 2
            return newStart,end
        else:
            newStart = (end - (((end - start) + 1) // 2)) + 1
            return newStart,end
    elif charec =='L':
        if start == '0' and end == '7':
            newEnd = ((end + 1) // 2) - 1
            ...
            newEndVal = (end-(((end-start)+1)//2))
            return start,newEndVal
